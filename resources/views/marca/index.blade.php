...
@section('content_header')
    <h1>LISTADO DE MARCAS Y MODELOS</h1>
@stop

@section('content')
@can('marca.create')
<a href="marcas/create" class="btn btn-primary mb-3">CREAR</a>
@endcan
<table id="marcas" class="table table-dark table-striped mt-4">
    <thead>
        <tr>
            <th scope="col">ID</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{$marca->condicion}}</td>
            
            <td>
                <form action="{{route ('marcas.destroy', $marca->id)}}" class="formulario-eliminar" method="POST">
                   @can('marca.show')
                   <a href="{{route('marcas.show', $marca->id)}}" class="btn btn-info"><i class="fas fa-info-circle"></i></a>
                   @endcan
                   @can('marca.edit')
                   <a href="marcas/{{$marca->id}}/edit" class="btn btn-primary" ><i class="far fa-edit"></i></a>
                   @endcan
                   @csrf
                   @method('DELETE')
                   @can('marca.destroy')
